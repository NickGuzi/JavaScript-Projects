...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //This condition checks who's sturn it is 
        if (activePlayer === "X") {
            //If activePlayer is equal to X, the x.png is placed in HTML
            select.style.backgroundImage = 'url("./images/x.png")'
            //Active player may be only X or O so, if not X it must be O
        } else {
        ...
        if (activePlayer === "X") {
            //If active player is the X change it to O
            activePlayer = "O";
    ...
    function computersTurn() {
        ...
        let pickASquare;
        //This confition allows for our while loop to keep trying if a square is selected already
        while (!success) {
            //A random number between 0 and 8 is selected
            pickASquare = String(Math.floor(Math.random() * 9));
            //If the number evaluated returns true, the square hasn't been selected yet
...
function checkWinConditions() {
    ...
    //This function checks if an array includes 3 strings. It is used to
...
function disableClick() {
    ...
    //This makes our bosy clickable again after 1 second
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y1 = coordY1,
        //This line indicates where the ed of a lines x axis is
        x2 = coordX2,
    ...
    function animateLineDrawing() {
        ...
        c.strokeStyle = "rgba(245, 229, 39, .8)"
